Auth 상태(로그인/유저) 사용 가이드 (Zustand persist = Source of Truth)

이 프로젝트의 인증 상태는 **Zustandustand persist(authStore)**가 관리합니다.
...
유저 정보/인증 필요 API(Query)는 hydrated && isLoggedIn일 때만 실행한다. (enabled 필수)

로그인 성공 시 actions.setAccessToken(token)만 호출한다.
→ persist가 localStorage 저장까지 처리

localStorage.getItem("accessToken")로 화면 가드/요청 제어 금지

setIsLoggedIn(true/false) 같은 수동 토글 금지 (구조상 필요 없음, 오류 유발 가능)
...
if (!hydrated) return null; // 또는 <Splash/>, <Skeleton/>
...
persist가 localStorage에서 값을 아직 복원 전일 수 있음

이 상태에서 Query를 실행하면 토큰이 없다고 판단 → 불필요한 실패/무한요청/경고 발생 가능

그래서 유저 인증이 필요한 Query는 무조건
